    ...
    <div className='flex flex-row items-center gap-10'>
       <div >
        <select
          ...
          className="block w-72 h-10 p-2 mt-1 text-buttonGray border-buttonGray rounded-md border-1 shadow-sm focus:ring-indigo-500 focus:border-gray-300 text-xxs bg-white"
        >
          <option className="text-buttonGray text-xxs">English</option>
          ...
          {Selectoptions[0]?.Secondoptions.map(option => (
            <option key={option.value} value={option.value} className="bg-white text-xxxs">
              {option.label}
            </option>
          ))}
        </select>
