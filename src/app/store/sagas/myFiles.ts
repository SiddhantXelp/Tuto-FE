...
import { getMyFiles, getMyFilesById, createFolder, createFiles, deleteFolders, deleteFiles, updateFolder, updateFile } from '../../api/myFiles.service';
import { setMyFiles, setMyFilesError, setMyFilesLoading, setMyFilesByID, setCreateFolder, setCreateFiles, setDeleteFolder, setUpdateFolder, setUpdateFiles } from '../actions/myFiles';
...
function* getUpdateFoldersEffect(action: any): Generator<any, any, any> {
    try {
        yield put(setMyFilesLoading(true));
        yield put(setMyFilesError(''));
        yield put(setUpdateFolder(null));

        const response = yield call(updateFolder, action.token, action.data, action.id);
        yield put(setUpdateFolder(response));
...
function* getUpdateFilesEffect(action: any): Generator<any, any, any> {
    try {
        yield put(setMyFilesLoading(true));
        yield put(setMyFilesError(''));
        yield put(setUpdateFiles(null));

        const response = yield call(updateFile, action.token, action.data, action.id);
        yield put(setUpdateFiles(response));
...
export function* MyFilesSaga() {
    ...
    yield takeEvery(actionTypes.GET_UPDATE_FOLDER, getUpdateFoldersEffect);
    yield takeEvery(actionTypes.GET_UPDATE_FILE, getUpdateFilesEffect);
